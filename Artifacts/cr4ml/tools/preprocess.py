...
class Preprocessor(object):
    ...
    def __init__(self, num_strategy="mean"):
        ...
        self.feature_enc = OneHotEncoder(handle_unknown='ignore')
    ...
    def fit(self, X_train, y_train, X_full=None, is_regression=False):
        ...
        self.indicator_num = len(nan_counts[nan_counts > 0].index)
        ...
        if len(self.cat_features) > 0:
            ...
            # print(self.cat_imputer.transform(X_train[self.cat_features].values))
            
            drop_element_transformer = FunctionTransformer(lambda X: drop_element(X, self.indicator_num))

            self.cat_transformer = Pipeline(steps=[
                ('imputer', self.cat_imputer),
                ('drop_last', drop_element_transformer), 
    ...
    def inverse_transform(self, X=None, y=None):
        ...
        if X is not None:
            ...
            if len(self.num_features) > 0:
                X_num_cols = len(self.num_features)
                X_num = X[:, :X_num_cols]
                X_num = self.num_transformer.inverse_transform(X_num)

            if len(self.cat_features) > 0:
                X_cat_cols = len(self.feature_enc.get_feature_names_out(self.cat_features))
                X_cat = X[:, -X_cat_cols:]
                
                X_cat = self.cat_transformer.named_steps['onehot'].inverse_transform(X_cat)
            ...
            if X_num is None:
                ...
                X_ans = np.concatenate((X_num, X_cat), axis=1)
                
            df_ans = pd.DataFrame(X_ans)
            df_ans.columns = self.num_features.tolist() + self.cat_features.tolist()

        if y is not None:
            y = self.label_enc.inverse_transform(y)
            df_class = pd.DataFrame(y)
            df_class.columns = self.class_feature.to_list()
...
def preprocess(data):
    ...
    # input two nparray, return two df
    # X_val_inverse, y_val_inverse = preprocessor.inverse_transform(X_val, y_val)
...
def onlypreprocess(X, y):
    ...
    X_transform = np.float32(X_transform)
    y_transform = np.float32(y_transform)
    # input two nparray, return two df
    # X_val_inverse, y_val_inverse = preprocessor.inverse_transform(X_val, y_val)

    return X_transform, y_transform, preprocessor

# def drop_last_column(df):
#     return df.drop(columns=[pd.IndexSlice[:, -1]])
def drop_element(arr, num):
    if num == 0:
        return arr
