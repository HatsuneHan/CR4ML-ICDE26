...
from tools.GLISTER.models.logistic_regression import LogisticRegNet
from tools.GLISTER.models.set_function_onestep import SetFunctionBatch
...
class CR4ML:
  ...
  def fit(self,
    ...
    while self.endflag != 1:
      ...
      if self.coreset_identifier == 'normal':
        ...
        self.idxT, self.idxV = self.getGlisterCoreset(0.25, 0.2, self.random_state, warmup_epochs=3, lr=0.05)
      else:
        raise ValueError("Invalid coreset identifier.")
  ...
  def getGlisterCoreset(self, budget_ratio, val_ratio, seed, warmup_epochs, lr):
    idxT, idxV = glister_coreset(
      X_train = self.X_train, y_train = self.y_train,
      cat_features = self.categorical_features, num_features = self.numerical_features,
      line_num = self.line_num, budget_ratio = budget_ratio, val_ratio = val_ratio,
      random_state = seed, standardize = True, eta = 0.05, warmup_epochs = warmup_epochs, lr = lr)
    return idxT, idxV
  ...
  def updateData(self, influ_tuples_dict, criti_attr_dict, iter_cnt):
    ...
    if (len(update_influ_idx) == 0 and iter_cnt > 5) or len(update_idx) == 0 or (iter_cnt >= self.max_epoch_cr4ml):
      self.endflag = 1
      return None, None
...
def glister_coreset(
  ...
  idxV = np.sort(idxV)
  idxT = np.setdiff1d(orig_idx, idxV)  
